...
import { Menu, X, CheckCircle2, DraftingCompass, HardHat, Ruler, Phone, Mail, MapPin, Building2, ChevronRight, Sun, Moon } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      >
        <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-2xl font-bold tracking-tight ${isScrolled ? 'text-slate-900 dark:text-white' : 'text-slate-900 md:text-white'}`}>
          ...
          <div className="flex items-center gap-4 lg:hidden">
            ...
            <button 
              className={`p-2 ${isScrolled ? 'text-slate-900 dark:text-white' : 'text-slate-900 md:text-white'}`}
        ...
        {/* Mobile Nav Overlay */}
        {mobileMenuOpen && (
          <div className="absolute top-full left-0 right-0 bg-white dark:bg-slate-900 shadow-xl p-4 flex flex-col gap-4 lg:hidden border-t border-slate-100 dark:border-slate-800 animate-in slide-in-from-top-2">
            {NAV_ITEMS.map((item) => (
              <a 
                key={item.label} 
                href={item.href}
                onClick={(e) => scrollToSection(e, item.href)}
                className="text-lg font-medium text-slate-800 dark:text-slate-200 py-2 border-b border-slate-50 dark:border-slate-800 last:border-0"
              >
                {item.label}
              </a>
            ))}
      ...
      <section id="home" className="relative h-screen flex items-center justify-center overflow-hidden bg-slate-900">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10 text-center text-white mt-16">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button variant="outline" size="lg" className="border-white text-white hover:bg-white hover:text-slate-900 bg-transparent dark:hover:bg-slate-800 dark:hover:text-white dark:hover:border-slate-800">
              Узнать больше
            </Button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
            {STATS.map((stat, idx) => (
              <div key={idx} className="text-center group">
                <div className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-white mb-2 group-hover:text-blue-600 transition-colors">
                  {stat.value}
                </div>
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {SERVICES.map((service) => (
              <div key={service.id} className="p-8 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 hover:shadow-xl hover:shadow-slate-200/80 dark:hover:shadow-slate-900/80 transition-all duration-300 hover:-translate-y-1 group">
                <div className="mb-6 bg-slate-50 dark:bg-slate-700 w-16 h-16 rounded-xl flex items-center justify-center group-hover:bg-blue-600/10 dark:group-hover:bg-blue-500/20 transition-colors">
                  {service.icon}
                </div>
                ...
                <a href="#contact" className="inline-flex items-center gap-2 mt-6 text-blue-600 dark:text-blue-400 font-medium hover:text-blue-700 dark:hover:text-blue-300 transition-colors">
      ...
      <section id="calculator" className="py-24 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
        <div className="container mx-auto px-4 md:px-6">
          <SectionHeader 
      ...
      {/* --- About Us Section --- */}
      <section id="about" className="py-24 bg-slate-50 dark:bg-slate-950 overflow-hidden transition-colors duration-300">
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="lg:w-1/2 relative">
              ...
              <div className="absolute -right-8 top-20 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-xl z-20 hidden md:block max-w-xs transition-colors">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-12 h-12 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center text-green-600 dark:text-green-400">
                    <CheckCircle2 size={24} />
                  </div>
                  <div>
                    <div className="font-bold text-slate-900 dark:text-white">Гарантия качества</div>
                    <div className="text-sm text-slate-500 dark:text-slate-400">ISO 9001 Сертификация</div>
                  </div>
                </div>
            ...
            <div className="lg:w-1/2">
              <span className="text-blue-600 dark:text-blue-400 font-bold tracking-wider uppercase mb-2 block">О Компании</span>
              ...
              <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-slate-700 dark:text-slate-200 font-medium">Собственный парк спецтехники</p>
                </div>
                <div className="flex items-start gap-3">
                  <div className="mt-1 w-2 h-2 rounded-full bg-blue-600 flex-shrink-0"></div>
                  <p className="text-slate-700 dark:text-slate-200 font-medium">Прямые контракты с производителями</p>
                </div>
                <div className="flex items-start gap-3">
                  <div className="mt-1 w-2 h-2 rounded-full bg-blue-600 flex-shrink-0"></div>
                  <p className="text-slate-700 dark:text-slate-200 font-medium">Юридическая чистота сделок</p>
                </div>
                <div className="flex items-start gap-3">
                  <div className="mt-1 w-2 h-2 rounded-full bg-blue-600 flex-shrink-0"></div>
                  <p className="text-slate-700 dark:text-slate-200 font-medium">Финансовая прозрачность</p>
                </div>
              </div>
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-start gap-4 transition-colors">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 dark:text-white text-lg mb-1">Телефон</h4>
                  <p className="text-slate-600 dark:text-slate-300 mb-1">+7 (495) 123-45-67</p>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Пн-Пт, 9:00 - 19:00</p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-start gap-4 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-slate-600 dark:text-slate-300 mb-1">info@stroyinvest.ru</p>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Отвечаем в течение часа</p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-start gap-4 transition-colors">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 dark:text-white text-lg mb-1">Офис</h4>
                  <p className="text-slate-600 dark:text-slate-300">г. Москва, Пресненская наб., 12</p>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Башня "Федерация"</p>
                </div>
              </div>
            </div>

            {/* Form */}
            <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-100 dark:border-slate-700 p-8 md:p-10 transition-colors">
              ...
              <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={(e) => e.preventDefault()}>
                <div className="space-y-2">
                  ...
                  <input type="text" id="name" className="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Иван Иванов" />
                </div>
                <div className="space-y-2">
                  <label htmlFor="phone" className="text-sm font-medium text-slate-700 dark:text-slate-300">Телефон</label>
                  <input type="tel" id="phone" className="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="+7 (___) ___-__-__" />
                </div>
                <div className="md:col-span-2 space-y-2">
                  <label htmlFor="message" className="text-sm font-medium text-slate-700 dark:text-slate-300">О проекте</label>
                  <textarea id="message" rows={4} className="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all resize-none" placeholder="Кратко опишите задачу..."></textarea>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="email" placeholder="Email" className="bg-slate-800 border-none rounded px-4 py-2 w-full focus:ring-2 focus:ring-blue-500 outline-none text-white placeholder-slate-500" />
